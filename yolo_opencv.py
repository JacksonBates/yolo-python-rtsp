...
# Addition needed to fix rtsp stream from my camera
os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;0"

ap = argparse.ArgumentParser()
ap.add_argument(
    "-i", "--input", required=False, help="path to input image", default="sampledata"
)
ap.add_argument(
    "-o",
    "--outputfile",
...
ap.add_argument(
    "-od", "--outputdir", required=False, help="path to output folder", default="output"
)
ap.add_argument("-fs", "--framestart", required=False, help="start frame", default=0)
ap.add_argument(
    "-fl",
    "--framelimit",
    required=False,
...
ap.add_argument(
    ...
    default="cfg/yolov3.txt",
)
ap.add_argument(
    "-ic", "--invertcolor", required=False, help="invert RGB 2 BGR", default="False"
)
ap.add_argument(
    "-fpt",
    "--fpsthrottle",
...
def save_bounded_image(image, class_id, confidence, x, y, x_plus_w, y_plus_h):
    ...
    dirname = os.path.join(
        args.outputdir, label, datetime.datetime.now().strftime("%Y-%m-%d")
    )
...
def handle_stream(cap):
    if int(args.framelimit) > 0:
        writer = imageio.write_frames(
            args.outputfile, (int(cap.get(3)), int(cap.get(4)))
        )
        writer.send(None)
    frame_counter = 0
    while True:
        if int(args.framelimit) > 0 and frame_counter > int(args.framestart) + int(
            args.framelimit
        ):
            writer.close()
            break
...
elif args.input.startswith("webcam"):
